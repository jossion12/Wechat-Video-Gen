...
⚠️ 当前版本(回退 Qwen3-TTS,DISABLED / DEPRECATED)⚠️
本模块全部功能已被 if False 包裹,不再生效。保留代码便于后续恢复 Qwen3-TTS 集成。
...
# ============================================================================
#  ⚠️ Qwen3-TTS 多角色对话合成 — DISABLED / DEPRECATED ⚠️
# ----------------------------------------------------------------------------
# 当前版本下述全部代码被 `if False:` 包裹,导入此模块不会执行任何
# env 读、单例创建、qwen_tts import、ASR 解析、合成调用。后续需要恢复
# Qwen3-TTS 集成时,把下面的 `if False:` 改成 `if True:`(或直接去掉 `if False:`)
# 即可,无需复原文档/集成逻辑。
# ============================================================================

if False:  # noqa: E800 — 见上方 DEPRECATED 注释
    # ---------- 配置(env) ----------
