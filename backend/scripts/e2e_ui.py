...
2. 手动添加 2 名参与者与 1 条消息 → 预览 iframe 渲染出默认群名「群聊」
3. 修改群名 → 预览 iframe 顶部标题同步更新(≤500ms 级别)
4. 点击「生成视频」→ 进度面板出现 → 等待「下载 MP4」按钮
5. 校验输出 URL 并下载 MP4,确认文件存在且可被 ffprobe 解析
"""
...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        # 1. 进入页面应为空白配置:无 demo 参与者/消息,标题为默认「群聊」
        ...
        ok = title_input.input_value() == "群聊"
        results.append(("默认标题为「群聊」", ok))
        print(f"  默认标题 = {title_input.input_value()!r} -> {'OK' if ok else 'FAIL'}")

        ok = preview_frame(page) is None
        results.append(("空配置不渲染预览 iframe", ok))
        print(f"  预览 iframe 存在 = {preview_frame(page) is not None} -> {'OK' if ok else 'FAIL'}")

        # 2. 添加 2 名参与者 + 1 条消息 → 预览渲染出默认群名
        page.locator("button:has-text('添加参与者')").click()
        page.locator("button:has-text('添加参与者')").click()
        ...
        results.append(("添加内容后预览 iframe 出现", ok))
        ...
        # 3. UI-1:修改群名 → 预览同步更新(iframe 会随预览重挂载,需每次重新取 frame)
        ...
        # 5. 下载 MP4 并校验(1 条消息 → 1200+1500+1500=4200ms=4.2s)
        ...
        ok = probe.returncode == 0 and abs(float(dur) - 4.2) < 0.5
        results.append(("MP4 时长≈4.2s", ok))
